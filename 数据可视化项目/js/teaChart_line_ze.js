(function() {
  ...
  highlightYear(year) {
      ...
      const index = this.teaData.years.indexOf(targetYear);
      
      if (index === -1) {
        console.warn(`未找到对应年份: ${year}`);
        return;
      }
      ...
      // 优化缩放逻辑
      const zoomSize = 3; // 显示当前年及前后各1年
      this.chart.dispatchAction({
        type: 'dataZoom',
        startValue: this.teaData.years[Math.max(0, index - zoomSize)],
        endValue: this.teaData.years[Math.min(this.teaData.years.length - 1, index + zoomSize)]
      });
